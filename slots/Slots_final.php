...
function winner(array $combinations, array $board): array
{
    $newCombo = [];
    foreach ($combinations as $index => $combination) {
        ...
        foreach ($combination as $combo => $item) {
            [$row, $column] = $item;
            $newCombo[$index][$combo] = $board[$row][$column];
        }
    }
    $winnercombo = [];
...
while ($totalCash >= 1) {
    ...
    switch ($f) {
        case 1 :
            ...
            $isWinner = winner($combinations, $board);

            if(sizeof($isWinner)>0) {
                foreach ($isWinner as $item) {
                    if ($item == '!') {
                        echo " You won " . ((int)$bet * $one * $spinCash) . " monies" . PHP_EOL;
                    ...
                    if ($item == '@') {
                        echo " You won " . ((int)$bet * $two * $spinCash) . " monies" . PHP_EOL;
